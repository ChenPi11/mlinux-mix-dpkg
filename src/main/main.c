...
"Note that the meanings and values are subject to change.\n"));
	...
	if (mask < 0)
		badusage(_("--%s requires a positive octal argument"),
		         cpi->olong);
}
...
arch_remove(const char *const *argv)
{
	...
	if (arch->type != DPKG_ARCH_FOREIGN) {
		warning(_("cannot remove non-foreign architecture '%s'"),
		        arch->name);
		return 0;
	}
...
int
execbackend(const char *const *argv) DPKG_ATTR_NORET;
int
...
commandfd(const char *const *argv)
{
	...
	for (;;) {
		...
		do {
			...
			/* This isn't fully accurate, but overestimating
			 * can't hurt. */
			if (c_isspace(c))
				argc++;
		} while (c != EOF && c != '\n');
