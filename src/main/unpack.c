	...
	if (!pid) {
		...
		if (!(WIFEXITED(status) && WEXITSTATUS(status) == 0)) {
			if (!in_force(FORCE_BAD_VERIFY))
				ohshit(_("verification on package %s failed!"),
				       filename);
			else
...
get_control_dir(char *cidir)
{
	if (!f_act) {
		...
		/* The admindir length is always constant on a dpkg execution
		 * run. */
		if (cidir == NULL)
			cidir = m_malloc(strlen(admindir) +
...
pkg_check_depcon(struct pkginfo *pkg, const char *pfilename)
{
	...
	/* Check if anything is installed that we conflict with, or not
	 * installed that we need. */
	ensure_package_clientdata(pkg);
	pkg->clientdata->istobe = PKG_ISTOBE_INSTALLNEW;

	for (dsearch = pkg->available.depends;
	     dsearch;
	...
	for (psearch = pkg->set->depended.installed;
	     psearch;
	     psearch = psearch->rev_next) {
		if (psearch->up->type != dep_conflicts)
			continue;
	...
	while ((de = readdir(dsd))) {
		...
		if (strchr(de->d_name, '.')) {
			debug_at(dbg_scripts,
			         "tmp.ci script/file '%s' contains dot",
			         de->d_name);
			continue;
		}
		...
		if (strcmp(de->d_name, CONTROLFILE) == 0) {
			debug_at(dbg_scripts,
			         "tmp.ci script/file '%s' is control",
			         cidir);
			continue;
		}
		...
		debug_at(dbg_scripts,
		         "tmp.ci script/file '%s' installed as '%s'",
		         cidir, newinfofilename);
	}
...
static void
pkg_remove_conffile_on_upgrade(struct pkginfo *pkg,
                               struct fsys_namenode *namenode)
{
	struct pkginfo *otherpkg;
                     ...
                     struct fsys_namenode_queue *newconffiles)
{
	...
	/* Before removing any old files, we try to remove obsolete conffiles
	 * that have been requested to be removed during upgrade. These
	 * conffiles are not tracked as part of the package file lists, so
	 * removing them here means we will get their parent directories
	 * removed if not present in the new package without needing to do
...
pkg_disappear_others(struct pkginfo *pkg)
{
	...
	while ((otherpkg = pkg_hash_iter_next_pkg(iter)) != NULL) {
		...
		if (!pdep) {
			/* If we haven't found a reason not to yet, let's
			 * look some more. */
			for (providecheck = otherpkg->installed.depends;
				...
				for (pdep = providecheck->list->ed->depended.installed;
					...
					debug_at(dbg_veryverbose,
					         "cannot disappear (provides %s): %s",
...
/* TODO: Refactor to reduce nesting levels. */
static void
pkg_remove_files_from_others(struct pkginfo *pkg,
                             struct fsys_namenode_list *newfileslist)
{
	struct fsys_namenode_list *cfile;
	struct pkginfo *otherpkg;
	...
	for (cfile = newfileslist; cfile; cfile = cfile->next) {
		...
		if (cfile->namenode->divert &&
		    cfile->namenode->divert->useinstead) {
			divpkgset = cfile->namenode->divert->pkgset;
			if (divpkgset == pkg->set) {
				debug_at(dbg_eachfile,
				         "not overwriting any '%s' (overriding, '%s')",
				         cfile->namenode->name,
				...
				debug_at(dbg_eachfile,
				         ...
				         cfile->namenode->name,
				         divpkgset ? divpkgset->name : "<local>");
			}
		} else {
			divpkgset = NULL;
		...
		while ((otherpkg = fsys_node_pkgs_iter_next(iter))) {
			...
			/* A pkgset can share files between instances, so
			 * there's no point in rewriting the file that's
			 * already in place. */
			...
			if (cfile->namenode->flags & FNNF_NEW_CONFF)
				conffile_mark_obsolete(otherpkg,
				                       cfile->namenode);
			 ...
			 * (and any others in the same package) and then mark
			 * the package as requiring a reread. */
...
static void
pkg_remove_backup_files(struct pkginfo *pkg,
                        struct fsys_namenode_list *newfileslist)
{
	struct fsys_namenode_list *cfile;
...
process_archive(const char *filename)
{
	...
	/* We want to guarantee the extracted files are on the disk, so that
	 * the subsequent renames to the info database do not end up with old
	 * or zero length files in case of a system crash. As neither dpkg-deb
	 * nor tar do explicit fsync()s, we have to do them here.
	 * XXX: This could be avoided by switching to an internal tar
	 * extractor. */
	...
	trig_parse_ci(cidir, NULL, trig_cicb_statuschange_activate,
	              pkg, &pkg->available);
	 ...
	 * delete them if they seem to disappear completely. */
	...
	if (oldversionstatus == PKG_STAT_NOTINSTALLED) {
		...
		push_cleanup(cu_preinstupgrade, ~ehflag_normaltidy,
		             4, (void *)pkg, (void *)cidir, (void *)cidirrest,
	...
	if (oldversionstatus == PKG_STAT_HALFINSTALLED ||
	    oldversionstatus == PKG_STAT_UNPACKED) {
		/* Packages that were in ‘installed’ and ‘postinstfailed’ have
		 * been reduced to ‘unpacked’ by now, by the running of the
		 * prerm script. */
		pkg_set_status(pkg, PKG_STAT_HALFINSTALLED);
		modstatdb_note(pkg);
		push_cleanup(cu_postrmupgrade, ~ehflag_normaltidy,
		             1, (void *)pkg);
	...
	/* In case this was an architecture cross-grade, the in-core pkgset
	 * might be in an inconsistent state, with two pkginfo entries having
	 * the same architecture, so let's fix that. Note that this does not
	 * lose data, as the pkg.available member parsed from the binary
	 * should replace the to be cleared duplicate in the other instance. */
	for (otherpkg = &pkg->set->pkg;
	     otherpkg;
	     otherpkg = otherpkg->arch_next) {
		if (otherpkg == pkg)
	 ...
	 * The files list for the conflictor is still a little inconsistent
	 * in-core, as we have not yet updated the filename->packages mappings;
	 * however, the package->filenames mapping is. */
	while (!pkg_queue_is_empty(&conflictors)) {
		struct pkginfo *conflictor = pkg_queue_pop(&conflictors);

		/* We need to have the most up-to-date info about which files
		 * are what ... */
		ensure_allinstfiles_available();
